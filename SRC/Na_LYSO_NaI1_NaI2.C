...
void Na_LYSO_NaI1_NaI2(TString Fname="../DST/Na2_1.dst"){
  ...
  //  tree->Draw("-A4-A3:T34", "-A3<6e-8 && -A4<6e-8 && abs(T34+5e-9)<7e-9 && abs(T32)<2e-8 && T34!=0 ", "colz");
     ...
     tree->Draw("T34:T32", "abs(T34+4e-9)<1e-8 && abs(T32-7e-9)<1e-8 && T34!=0", "colz");
     tree->Draw("-A2/PK2:T32", "abs(T34+4e-9)<1e-8 && abs(T32-7e-9)<1e-8 && T34!=0 && -A2/PK2<0.11e-6", "colz");
      tree->Draw("-A2:-A3", "abs(T34+4e-9)<1e-8 && abs(T32-7e-9)<1e-8 && T34!=0 && -A2/PK2<0.11e-6", "colz");
   return;
